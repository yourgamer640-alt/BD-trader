...
<p class="text-center text-zinc-400 text-sm mb-6">Chart upload dao, Signal nao</p>
<select id="market" class="w-full bg-zinc-800 p-3 rounded-xl mb-3">
<option>EUR/USD</option><option>BTC/USD</option><option>GOLD</option><option>GBP/USD</option>
</select>
...
async function analyze(){
  ...
  if(!file){alert('Chobi dao vai!');return;}
  const btn=document.getElementById('btn');
  const resDiv=document.getElementById('result');
  btn.innerText='Analyzing...'; btn.disabled=true;
  resDiv.classList.remove('hidden');
  resDiv.innerText='AI chinta korche... 10 second lagbe';
  const form=new FormData();
  form.append('image',file);
  form.append('market',document.getElementById('market').value);
